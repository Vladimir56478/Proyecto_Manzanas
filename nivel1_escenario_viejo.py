...
class Game:
    ...
    def create_placeholder_images(self):
        ...
        self.victory = False
        self.enemies_defeated = 0
        self.total_worms_spawned = 0  # Contador total
        self.victory_condition = 15  # Derrotar 15 gusanos para ganar
    ...
    def process_worm_drops(self):
        """Procesa los drops de gusanos muertos con contador optimizado"""
        for worm in self.worm_spawner.get_worms():
            if hasattr(worm, 'health') and worm.health <= 0 and not getattr(worm, 'counted_as_defeated', False):
                # Marcar como contado para evitar múltiples conteos
                worm.counted_as_defeated = True
                self.enemies_defeated += 1
                
                # Generar drops solo una vez por gusano
                if not getattr(worm, 'drop_processed', False):
                    worm.drop_processed = True
                    
                    # Probabilidad de drop mejorada
                    drop_chance = random.random()
                    if drop_chance < 0.6:  # 60% probabilidad
                        drop_x = worm.x + random.randint(-30, 30)
                        drop_y = worm.y + random.randint(-30, 30)
                        
                        if drop_chance < 0.35:  # 35% manzana
                            self.dropped_items.append({
                                'type': 'apple',
                                'x': drop_x, 'y': drop_y,
    ...
    def update_collectibles(self):
        """Actualiza lógica de coleccionables"""
        current_time = pygame.time.get_ticks()
        
        # Eliminar items que han estado demasiado tiempo
        self.dropped_items = [item for item in self.dropped_items 
                            if current_time - item['spawn_time'] < 30000]  # 30 segundos
        
        # Verificar colisiones con personajes
        player_rect = pygame.Rect(self.active_character.x, self.active_character.y, 64, 64)
        ...
        for item in self.dropped_items[:]:
            ...
            # Colisión con personaje inactivo (si está vivo)
            elif self.inactive_character.health > 0 and inactive_rect.colliderect(item_rect):
                if item['type'] == 'apple':
                    self.collect_apple()  # Las mejoras aplican al personaje activo
                elif item['type'] == 'potion':
                    self.collect_potion(self.inactive_character)
                self.dropped_items.remove(item)
